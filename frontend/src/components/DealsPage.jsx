...
const PAGE_SIZE = 10;
const EMPTY_FORM = {
  name: "", description: "", client_id: "", stage_id: "", service_id: "", estimated_budget: "", deadline: "", manager_effort_estimate: "", probability: "", responsible_user_id: "",
};
...
function DealForm({ form, clients, stages, employees, services, categories, isAdmin, submitting, error, onChange, onSubmit, submitLabel, mode }) {
  ...
  return <form className="client-form" onSubmit={onSubmit}>
    <div className="client-form-grid">
      ...
      <label className="field"><span>{t("d4.service")}</span><select name="service_id" value={form.service_id} onChange={onChange} disabled={submitting} required><option value="">{t("d4.selectService")}</option>{services.filter((item) => (item.is_active && categories.find((category) => category.id === item.category_id)?.is_active) || item.id === form.service_id).map((item) => <option key={item.id} value={item.id}>{item[`name_${language}`] || item.name_ru}</option>)}</select></label>
      <label className="field"><span>{t("d4.category")}</span><input value={(() => { const item = categories.find((category) => category.id === services.find((service) => service.id === form.service_id)?.category_id); return item?.[`name_${language}`] || item?.name_ru || "—"; })()} disabled /></label>
      ...
      <label className="field"><span>{t("d4.managerEffort")}</span><input name="manager_effort_estimate" type="number" min="0.01" step="0.01" value={form.manager_effort_estimate} onChange={onChange} disabled={submitting} /></label>
...
function DealsPage({ initialDealId }) {
  ...
  const [references, setReferences] = useState({ clients: [], stages: [], employees: [], services: [], categories: [] });
  ...
  const load = useCallback((nextOffset = offset) => {
    ...
    Promise.all([
      ...
      listServices(accessToken, controller.signal), listCategories(accessToken, controller.signal),
    ]).then(([nextDeals, clients, stages, employees, services, categories]) => {
      setDeals(nextDeals); setReferences({ clients, stages, employees, services, categories }); setOffset(nextOffset); setState("ready");
  ...
  const serviceLabel = (serviceId) => references.services.find((item) => item.id === serviceId)?.[`name_${locale}`] ?? t("deals.unknownReference");
  const categoryLabel = (serviceId) => { const service = references.services.find((item) => item.id === serviceId); return references.categories.find((item) => item.id === service?.category_id)?.[`name_${locale}`] ?? t("deals.unknownReference"); };
  const hasInactiveBusinessReference = (serviceId) => { const service = references.services.find((item) => item.id === serviceId); const category = references.categories.find((item) => item.id === service?.category_id); return Boolean(service && (!service.is_active || !category?.is_active)); };

  function openCreate() {
    setForm({ ...EMPTY_FORM, client_id: references.clients[0]?.id ?? "", stage_id: references.stages[0]?.id ?? "", service_id: references.services.find((item) => item.is_active && references.categories.find((category) => category.id === item.category_id)?.is_active)?.id ?? "" });
  ...
  function openEdit(deal) {
    setForm({ name: deal.name, description: deal.description ?? "", client_name: clientLabel(deal.client_id), stage_name: stageLabel(deal.stage_id), service_id:deal.service_id ?? "", estimated_budget: deal.estimated_budget ?? "", deadline: deal.deadline ?? "", manager_effort_estimate:deal.manager_effort_estimate ?? "", probability: deal.probability ?? "", responsible_user_id: deal.responsible_user_id ?? "" });
  ...
  function payloadFor(mode) {
    const payload = {
      ...
      service_id: form.service_id || null, manager_effort_estimate: form.manager_effort_estimate === "" ? null : form.manager_effort_estimate,
    };
    if (mode === "create") {
      payload.client_id = form.client_id; payload.stage_id = form.stage_id;
      payload.ai_analysis_language = (i18n.resolvedLanguage ?? "ru").toUpperCase();
  ...
  return <>
    ...
    {modal && <div className="modal-backdrop" role="presentation"><section className="client-modal" role="dialog" aria-modal="true" aria-labelledby="deal-modal-title">
      <div className="modal-header"><div><p className="eyebrow">{modal.mode === "create" ? t("deals.create") : modal.mode === "edit" ? t("deals.edit") : t("deals.details")}</p><h2 id="deal-modal-title">{modal.deal?.name ?? t("deals.create")}</h2></div><button className="icon-button" type="button" onClick={() => setModal(null)} aria-label={t("common.close")}>×</button></div>
      {modal.mode === "loading" ? <div className="state-panel"><span className="loading-spinner" /><p>{t("deals.loading")}</p></div> : modal.mode === "detail" ? <>
        <dl className="deal-details"><div><dt>{t("deals.fields.client")}</dt><dd>{clientLabel(modal.deal.client_id)}</dd></div><div><dt>{t("deals.fields.stage")}</dt><dd>{stageLabel(modal.deal.stage_id)}</dd></div><div><dt>{t("d4.service")}</dt><dd>{serviceLabel(modal.deal.service_id)}</dd></div><div><dt>{t("d4.category")}</dt><dd>{categoryLabel(modal.deal.service_id)}</dd></div><div><dt>{t("deals.fields.responsible")}</dt><dd>{employeeLabel(modal.deal.responsible_user_id)}</dd></div><div><dt>{t("deals.fields.estimated_budget")}</dt><dd>{modal.deal.estimated_budget ?? "—"}</dd></div><div><dt>{t("deals.fields.deadline")}</dt><dd>{dateLabel(modal.deal.deadline, locale)}{modal.deal.deadline && modal.deal.deadline < new Date().toISOString().slice(0,10) && <small className="deadline-warning">{t("d4.pastDeadline")}</small>}</dd></div><div><dt>{t("d4.managerEffort")}</dt><dd>{modal.deal.manager_effort_estimate ? `${modal.deal.manager_effort_estimate} h · ${Number(modal.deal.manager_effort_estimate)/8} d` : "—"}</dd></div><div><dt>{t("deals.fields.probability")}</dt><dd>{modal.deal.probability == null ? "—" : `${modal.deal.probability}%`}</dd></div><div className="field--full"><dt>{t("deals.fields.description")}</dt><dd>{modal.deal.description || "—"}</dd></div></dl>
        {hasInactiveBusinessReference(modal.deal.service_id) && <p className="warning-box">{t("d4.inactiveBusinessReference")}</p>}
        <LeadScoringSection deal={modal.deal} canRun={canChange(modal.deal)} isClosed={["Won","Lost"].includes(references.stages.find((item) => item.id === modal.deal.stage_id)?.name)} />
        <DealPredictionSection deal={modal.deal} canRun={canChange(modal.deal)} isClosed={["Won","Lost"].includes(references.stages.find((item) => item.id === modal.deal.stage_id)?.name)} />
        <NextBestActionSection deal={modal.deal} canRun={canChange(modal.deal)} isClosed={["Won","Lost"].includes(references.stages.find((item) => item.id === modal.deal.stage_id)?.name)} />
        <EmailDraftSection deal={modal.deal} canRun={canChange(modal.deal)} />
