...
function DealAIHistorySection({ dealId }) {
  ...
  return <details className="next-best-action deal-ai-history"><summary><strong><History aria-hidden="true" />{t("aiHistory.dealTitle")}</strong></summary>{error ? <p className="form-error">{t("aiHistory.error")}</p> : !page ? <p>{t("aiHistory.loading")}</p> : <AIHistoryList items={page.items} />}</details>;
}
