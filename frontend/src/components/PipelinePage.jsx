...
function moneyLabel(value, locale) { return new Intl.NumberFormat(locale, { style: "currency", currency: "EUR" }).format(Number(value)); }
function percentLabel(value, locale) { return new Intl.NumberFormat(locale, { style: "percent", maximumFractionDigits: 0 }).format(Number(value) / 100); }
...
function PipelinePage({ onOpenDeal }) {
  ...
  return <>
    ...
    {state === "ready" && data.deals.length > 0 && <section className="kanban-scroll" aria-label={t("pipeline.boardLabel")}><div className="kanban-board">{data.stages.map((stage) => {
      ...
      return <section className={`kanban-column ${draggedDeal && draggedDeal.stage_id !== stage.id && canMove(draggedDeal) ? "is-drop-target" : ""}`} key={stage.id} onDragOver={(event) => { if (draggedDeal && canMove(draggedDeal)) event.preventDefault(); }} onDrop={(event) => drop(event, stage.id)}><header className="kanban-column-header"><h2>{pipelineStageLabel(t, stage)}</h2><span>{t("pipeline.dealCount", { count: deals.length })}</span></header><div className="kanban-cards">{deals.length === 0 && <p className="readonly-note kanban-stage-empty">{t("pipeline.emptyStage")}</p>}{deals.map((deal) => <article className={`kanban-card ${canMove(deal) ? "" : "is-readonly"}`} key={deal.id} draggable={canMove(deal)} onDragStart={(event) => dragStart(event, deal)} onDragEnd={() => setDraggedDeal(null)} onClick={() => onOpenDeal(deal.id)} title={canMove(deal) ? t("pipeline.cardMoveHint") : t("pipeline.readonlyHint")}><h3>{deal.name}</h3><p>{clientLabel(deal)}</p><dl><div><dt>{t("deals.fields.responsible")}</dt><dd>{employeeLabel(deal)}</dd></div>{deal.estimated_budget != null && <div><dt>{t("deals.fields.estimated_budget")}</dt><dd>{moneyLabel(deal.estimated_budget, locale)}</dd></div>}{deal.probability != null && <div><dt>{t("deals.fields.probability")}</dt><dd>{percentLabel(deal.probability, locale)}</dd></div>}{deal.deadline && <div><dt>{t("deals.fields.deadline")}</dt><dd>{formatDeadline(deal.deadline, locale)}</dd></div>}</dl>{canMove(deal) ? <button className="text-button kanban-move-button" type="button" onClick={(event) => { event.stopPropagation(); setMoveDeal(deal); }}>{t("pipeline.move")}</button> : <small>{t("pipeline.readonly")}</small>}</article>)}</div></section>;
    })}</div></section>}
