...
from backend.app.models import AIAnalysis, AIAnalysisStatus, AIFunctionType, AIResultLanguage, Deal, EmailDraft, User, UserRole
...
def list_email_drafts(session: Session, *, deal_id: UUID, current_user: User) -> list[EmailDraft]:
    ...
    return list(session.scalars(select(EmailDraft).where(EmailDraft.deal_id == deal_id).order_by(EmailDraft.updated_at.desc(), EmailDraft.id.desc())))
