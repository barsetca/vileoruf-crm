...
def list_unmatched_external_messages(session: Session, *, limit: int, offset: int) -> list[ExternalMessage]:
...
def link_unmatched_external_message(
    session: Session, *, external_message_id: UUID, client_id: UUID, current_user: object
) -> ExternalMessage:
    ...
    if not message or message.direction != "INCOMING" or not session.get(Client, client_id):
        raise InboxError
